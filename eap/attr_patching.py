...
def node_attribution_patching(
    ...
    Args:
        model (lens.HookedTransformer): The model to be analyzed.
        prompts_and_answers (List[Tuple[str, str]]): A list of tuples containing prompts and answers.
        corrupt_prompts_and_answers (List[Tuple[str, str]], optional): A list of tuples containing corrupt prompts and answers.
                If mean ablations are used via the mean cache. Defaults to None.
        mean_cache (Dict[Component, torch.Tensor], optional): A dictionary containing the mean cache of the model. If corrupt_prompts_and_answers is not given,
                this cache is used to calculate the ablation. Defaults to None.
        attributed_hook_names (List[str], optional): A list of hook names to be attributed. Defaults to ['mlp.hook_post', 'hook_z'].
        metric (str, optional): The metric to be used for the attribution. Defaults to 'IE' (indirect effect).
        batch_size (int, optional): The batch size to be used for the attribution. Defaults to 1.
        verbose (bool, optional): Whether to print a progress bar. Defaults to True.
    Returns:
        Dict[str, torch.Tensor]: A dictionary containing the attribution patching scores for each component.
            Each score tensor has the shape (len(prompts), *hook_dim).
    """
    ...
    assert (
        corrupt_prompts_and_answers or mean_cache
    ), "Either corrupt prompts or mean cache must be provided for ablation attribution"
    ...
    it = (
        tqdm(range(0, len(prompts), batch_size))
        if verbose
    ...
    for idx in it:
        ...
        if use_counterfactual_ablation:
            ...
            corrupt_label_batch = corrupt_labels[idx : idx + batch_size].to(
        ...
        def backward_hook_fn(grad, hook, attr_patching_scores):
            ...
            if hook.name not in attr_patching_scores:
                attr_patching_scores[hook.name] = torch.zeros(
                    (len(prompts),) + clean_cache[hook.name].shape[1:], device="cpu"
                )
            attr_patching_scores[hook.name][idx : idx + batch_size] = (
                diff_cache[hook.name] * grad
            ).cpu()
        ...
        with torch.set_grad_enabled(True):
            ...
            if metric == "IE":
                if use_counterfactual_ablation:
                    ...
                    value = (
                        pre_ablation_probs.gather(1, label_batch)
                        - post_ablation_probs.gather(1, label_batch)
                    ) / post_ablation_probs.gather(1, label_batch)
