...
class EAPGraph:
    ...
    def get_edges_with_scores(self):
        assert self.eap_scores is not None, "EAP scores have not been computed yet"

        sorted_indices = torch.sort(self.eap_scores.flatten(), dim=0).indices
        upstream_node_indices, downstream_node_indices = np.unravel_index(sorted_indices, self.eap_scores.shape)
        scores = self.eap_scores[upstream_node_indices, downstream_node_indices]
        top_edges = [(self.upstream_nodes[upstream_node_indices[i]], self.downstream_nodes[downstream_node_indices[i]], scores[i].item()) for i in range(len(scores))]

        # top_edges = []
        # for _, (_, index) in enumerate(zip(top_scores, top_indices)):
    ...
    def show(
        ...
        for edge in edges:
            ...
            if 'resid_post' in child_node and child_node != f'resid_post.{model.cfg.n_layers - 1}':
                continue
            ...
            edge_width = str(max(minimum_penwidth, edge_score*5))
