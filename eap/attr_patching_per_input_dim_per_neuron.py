...
def full_attribution_patching_per_input_dim_per_neuron(model: lens.HookedTransformer,
    ...
    forward_hook_filter = partial(should_measure_hook, measurable_hooks=forward_hook_names)
    backward_hook_filter = partial(should_measure_hook, measurable_hooks=backward_hook_names)
    ...
    labels, corrupt_labels = model.to_tokens(answers, prepend_bos=False), model.to_tokens(corrupt_answers, prepend_bos=False)

    attr_patching_scores = {}

    for idx in tqdm(range(0, len(prompts), batch_size)):
        prompt_batch, corrupt_prompt_batch = prompts[idx : idx + batch_size], corrupt_prompts[idx : idx + batch_size]
        label_batch, corrupt_label_batch = labels[idx : idx + batch_size], corrupt_labels[idx : idx + batch_size]

        # First forward pass to get corrupt cache
        _, corrupt_cache = model.run_with_cache(corrupt_prompt_batch)
        corrupt_cache = {k: v for (k, v) in corrupt_cache.cache_dict.items() if forward_hook_filter(k)}

        # Second forward pass to get corrupt cache
        clean_logits_orig, clean_cache = model.run_with_cache(prompt_batch)
        clean_cache = {k: v for (k, v) in clean_cache.cache_dict.items() if forward_hook_filter(k)}
        ...
        def backward_hook_fn(grad, hook, attr_patching_scores):
            ...
            if matching_hook_name_key not in attr_patching_scores:
                ...
                attr_patching_scores[matching_hook_name_key] = torch.zeros(len(prompts), model.cfg.d_model, model.cfg.d_mlp, device='cpu')  # for per neuron per input dim attribution
            ...
            attr_patching_scores[matching_hook_name_key][idx : idx + batch_size] = (diff_cache[matching_hook_name_key].unsqueeze(-1) * grad_L_wrt_e)


        model.reset_hooks()
        model.add_hook(name=backward_hook_filter, hook=partial(backward_hook_fn, attr_patching_scores=attr_patching_scores), dir="bwd")
        with torch.set_grad_enabled(True):   
            clean_logits = model(prompt_batch, return_type="logits")
            if metric == 'IE':
                # TODO THINK ABOUT THE MEANING OF USING "indirect effect" HERE WHEN THERE REALLY ISNT ANY DIFFERENCE BETWEEN THE CLEAN LOGITS AND THE "PATCHED" LOGITS
                value = indirect_effect(clean_logits_orig[:, -1].softmax(dim=-1), clean_logits[:, -1].softmax(dim=-1), label_batch, corrupt_label_batch).mean(dim=0)
            else:
